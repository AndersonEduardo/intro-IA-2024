  ...
  while True:
    ...
    for contour in contours:
      ...
      if w > 20 and h > 20:
        ...
        # Resize ROI to 8x8
        resized_roi = cv2.resize(roi, (40, 40), interpolation=cv2.INTER_AREA)

        # Flatten ROI
        resized_roi = np.expand_dims(resized_roi, axis=-1)  # Adicionar um canal de cor (RGB)
        resized_roi = np.repeat(resized_roi, 3, axis=-1)  # Replicar o canal de cor 3 vezes para simular RGB
        flattened_roi = resized_roi.reshape(-1, 40, 40, 3)

        # Predict digit using model
        prediction = model.predict(flattened_roi)

        # Display predicted digit on frame
        cv2.putText(frame, str(prediction.argmax()), (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
